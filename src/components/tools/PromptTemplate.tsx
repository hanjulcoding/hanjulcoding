...
const PromptTemplate: React.FC = () => {
  ...
  const save = () => {
    ...
    const kept = Object.fromEntries(
      vars.filter((v) => replacements[v]).map((v) => [v, replacements[v]!])
    );
                ...
                <div className="mt-4">
                  ...
                  {vars.length === 0 ? (
                    <p className="text-sm text-gray-400 italic">
                      본문에{" "}
                      <code className="rounded bg-[rgb(var(--foreground))]/10 px-1">
                        {"{{변수}}"}
                      </code>{" "}
                      를 추가하면 여기서 값을 채울 수 있어.
                    </p>
                  ) : (
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {vars.map((v) => (
                            <tr
                              key={v}
                              className="border-b border-[rgb(var(--foreground))]/10 last:border-0"
                            >
