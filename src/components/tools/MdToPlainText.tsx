            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <label className="flex cursor-pointer items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={removeBold}
                  onChange={(e) => setRemoveBold(e.target.checked)}
                />
                굵은 글씨(Bold) 문법 제거
              </label>
              <label className="flex cursor-pointer items-center gap-2 text-sm">
            ...
            <div className="mt-3">
              ...
              <div
                className={
                  replaceRulesEnabled
                    ? "mt-2 space-y-2"
                    : "mt-2 space-y-2 pointer-events-none opacity-50"
                }
              >
                {replaceRules.map((rule, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setReplaceRules((prev) =>
                          prev.map((r, i) => (i === index ? { ...r, from: e.target.value } : r))
                        )
                      }
                      className={`${inputBox} text-sm`}
                    />
                    <input
                      ...
                      onChange={(e) =>
                        setReplaceRules((prev) =>
                          prev.map((r, i) => (i === index ? { ...r, to: e.target.value } : r))
                        )
                      }
                      className={`${inputBox} text-sm`}
                    />
